...
function Like(props) {
  ...
  const dislikePost = async () => {
    axiosPrivate
      ...
      .then((res) => {
        console.log('Dislikes Response:', res.data);
        getDislikes();
        // if (res.status === 200) {
        //   setdisLikedAction(true);
        //   setLikedAction(false);
  ...
  return (
    <React.Fragment>
      <div className='flex align-middle items-center justify-start gap-2 flex-row '>
        <button>
          <svg
            xmlns='http://www.w3.org/2000/svg'
            className={
              'h-4 w-4' +
              (likedAction ? ' fill-red-800' : 'fill-black dark:fill-white')
            }
            viewBox='0 0 16 16'
            onClick={() => {
              likePost();
            }}
            disabled={likedAction}
          >
        ...
        <button
          ...
          disabled={dislikedAction}
        >
          <svg
            xmlns='http://www.w3.org/2000/svg'
            className={
              'h-4 w-4' +
              (dislikedAction ? ' fill-red-800' : 'fill-black dark:fill-white')
            }
            viewBox='0 0 16 16'
          >
